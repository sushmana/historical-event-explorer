...
const Modal = (props) => {
  ...
  return (
    ...
    <div className={`fixed inset-0 flex justify-center items-center transition-colors ${props.openModal ? "visibile bg-white/20": "invisible"}`}>
      <div onClick={(e)=>{e.stopPropagation() }} className={` mx-[20px] w-auto h-[500px] overflow-y-auto overflow-hidden bg-white rounded-xl shadow p-2 transition-all ${props.openModal ? "scale-100 opacity-95": "scale-125 opacity-0"}`}>
